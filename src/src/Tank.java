...
public class Tank extends GameObject {
    ...
    void setGW(RUNTHIS gw_to_set) { //this is needed for spawning bullets
        this.gw = gw_to_set;
    }
    ...
    private void checkBorder() {
        ...
        if (x >= RUNTHIS.WORLD_WIDTH - 88) {
            x = RUNTHIS.WORLD_WIDTH - 88;
        }
        if (y < 40) {
            y = 40;
        }
        if (y >= RUNTHIS.WORLD_HEIGHT - 80) {
            y = RUNTHIS.WORLD_HEIGHT - 80;
        }
    }


    private void SpawnBullet(int x, int y, int vx, int vy, int angle, RUNTHIS gw) { //private access because only the tank is able to spawn bullets
